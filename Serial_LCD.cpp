...
uint8_t Serial_LCD::setSpeed(uint32_t speed) {
  ...
  else if (speed==(uint16_t) 38400) a=0x0a;  // max for Arduino and software serial
  else if (speed==(uint16_t) 57600) a=0x0c;  
  else if (speed==(uint16_t)115200) a=0x0d;  // ok with chipKIT and Arduino mega hardware serial
...
uint8_t Serial_LCD::setFont(uint8_t size) {
  ...
  if ( size>3 ) size=3;
  if ( (size>2) && (_checkedScreenType==__Goldelox__) ) size=2;
